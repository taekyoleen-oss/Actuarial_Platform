...
| 2026-07-10 | 지구본 대륙을 점묘→실제 지도(대륙면+해안선)로 + 중앙 문구 옅게 | GlobeBackdrop, package.json(d3-geo·topojson-client) | 사용자 요청: 진짜 지구본 지도 형태 — d3-geo 정사영(geoOrthographic, clipAngle 90)으로 land-110m을 채운 대륙면(rgba primary 0.2)+해안선(0.5)으로 렌더. 커스텀 proj와 d3 rotate([lam,-TILT]) 좌표 정합 수식 검증(노드·아크가 지도 위 정위치). 경위선도 d3 graticule로 대체, 점묘 코드(pointInLand·topoToLandDots) 제거. 중앙 문구는 CENTER_ALPHA 0.4로 분리(옅게, halo 제거) |
