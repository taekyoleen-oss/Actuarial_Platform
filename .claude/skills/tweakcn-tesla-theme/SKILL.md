...
description: "Tesla 미니멀 디자인 토큰을 globals.css CSS 변수와 shadcn/TweakCN 테마 변수로 생성·매핑하는 스킬. 강조색 1개(#3E6AE1)+브랜드 마크·카드 타이틀 한정 sky, 그림자는 카드 엘리베이션(--shadow-card)만, 색상 트랜지션 0.33s+카드 lift 한정 예외, Pretendard/Inter 400·500+카드 타이틀 600(--brand-sky) 규칙을 강제한다. 테마 초기화, 색상/토큰 변경, globals.css 작성, shadcn 컴포넌트 커스터마이징, Tesla 톤 적용 시 반드시 사용. 디자인 토큰 수정·보완 요청 시에도 사용."
---
...
- **본문·UI는 weight 400/500만**. 위계는 크기·여백으로 만든다. 단, **카드 타이틀은 Pretendard(고딕) 600 + `--brand-sky`(로고 블루)** — 설명 텍스트와 굵기·색으로 구별. (세리프 안은 2026-06-11 사용자 결정으로 고딕으로 변경)
- 폰트: Pretendard(한글/UI) + Inter(라틴 폴백). letter-spacing normal, 대문자 변환 없음. 별도 세리프 로딩 없음.
- 페이지 캔버스는 `--page-bg`(#FAFAF7, 브랜드 cream), 카드 표면은 화이트 — 엘리베이션이 읽히는 바탕.

### v2 입체화 완화 (2026-06-13, 사용자 승인 — 깊이감·몰입감 강화)
콘텐츠 밀도가 높은 자료실의 가독성을 위해 아래를 **한정 스코프**로 허용한다. 그라데이션 금지·단일 강조색 원칙은 유지.
- **헤딩 무게 600/700 대비**: 페이지 h1/섹션 타이틀에 `font-semibold`/`font-bold` 허용(본문은 400/500, 300 금지). tailwind `fontWeight`에 semibold/bold 추가.
- **글래스 패널** `.glass-panel`(반투명 white + `backdrop-blur`): **스티키 필터바·내비/TOC 한정**. 일반 카드엔 금지.
- **데이터 워터마크** `.bg-watermark-curve`(정규분포+산점 SVG, 불투명도 ≤5%): **히어로·섹션 헤더 배경 한정**. `.bg-actuarial-dots`와 병용 가능.
- **칩 뮤트 팔레트 8색** `--chip-{blue,teal,amber,rose,violet,green,cyan,slate}-{bg,fg}`: **FSA 테마칩·차트 시리즈·아이콘 배경 한정**. 본문 텍스트/버튼엔 금지.
- **플로팅 그림자** `--shadow-float`(`shadow-float`): 스티키 바 한정. 카드는 계속 `--shadow-card`.
- **마이크로 모션**: 카운트업(`StatStrip`)·스크롤 리빌(`.reveal-fade`→`.in`, `Reveal`)·탭 페이드(`.tab-fade-in`)·핵심 구절 하이라이트(`mark.key-phrase`, `--mark-bg`)·용어 점선 밑줄(`.term-trigger`). 모두 `prefers-reduced-motion: reduce` 가드 필수.
...
| Card | 화이트 + `--shadow-card` 엘리베이션, hover lift(-4px)+`--shadow-card-hover`. 타이틀은 Pretendard 600·`--brand-sky`(hover `--primary`). 커버형은 radius 12px + overflow hidden |
