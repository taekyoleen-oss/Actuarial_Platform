...
function buildUserMessage(input: PyAssistInput): string {
  ...
  if (input.mode === "vars") {
    ...
    return [
      "다음 셀 코드에서 사용하는 '변수 이름'만 조정하세요. 아래 '데이터 스키마'의 vars 키(=현재 세션에 실제로 존재하고 앞 셀에서 만든 변수)에 있는 변수로, 이 셀의 데이터프레임 등 변수 이름을 바꿉니다.",
      target
        ? `사용자가 대상 변수를 지정했습니다: 이 셀의 데이터프레임 변수를 반드시 '${target}'(으)로 바꾸세요.`
        : "여러 후보가 있으면, 이 셀이 참조하는 열 이름과 가장 잘 맞는 변수를 고르세요.",
      "규칙: 코드의 로직·구조·함수·열 이름·문자열은 그대로 두고, 오직 변수 이름만 실제 존재하는 변수로 교체합니다. 이미 올바르면 그대로 두세요. 스키마의 vars에 없는 이름으로는 절대 바꾸지 마세요.",
      "",
