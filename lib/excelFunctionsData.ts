...
export const EXCEL_FUNCTIONS: ExcelFunction[] = [
  ...
  },
  {
    ...
    "intro": "SORT와 SORTBY는 둘 다 원본 데이터를 건드리지 않고 정렬된 결과만 새 위치에 좌르륵 펼쳐(스필) 주는 자동 정렬 함수입니다. 원본이 바뀌면 정렬 결과도 자동으로 다시 계산되어 항상 최신 상태로 유지됩니다. 차이는 '무엇을 기준으로 정렬하느냐'에 있습니다.\n\nSORT는 정렬 대상 표 안의 '몇 번째 열'을 기준으로 삼습니다. =SORT(배열, 정렬기준, 정렬방향)처럼 기준으로 쓸 열 번호와 오름차순(1)·내림차순(-1)을 지정합니다. 네 번째 인수(열기준)를 TRUE로 주면 세로가 아니라 가로 방향으로도 정렬할 수 있습니다. 기준이 되는 열이 결과에 함께 나와도 상관없을 때 가장 간단합니다.\n\nSORTBY는 정렬 기준을 결과에 포함하지 않아도 되는 정렬 함수입니다. 결과로는 '이름 목록'만 받고 싶은데 정렬은 '청구액 크기순'으로 하고 싶을 때 =SORTBY(이름범위, 청구액범위, -1)처럼 아예 다른 범위나 계산식을 기준으로 삼습니다. 게다가 (기준, 방향) 짝을 계속 이어 붙여 '지역 오름차순 → 같은 지역 안에서는 청구액 내림차순' 같은 다단계 정렬을 쉽게 하고, 손해율(청구액÷보험료) 같은 계산식을 기준으로도 정렬할 수 있습니다.\n\n정리하면, 표 안의 열 번호 하나로 간단히 정렬할 때는 SORT, 결과에 없는 다른 범위·계산식을 기준으로 삼거나 다중 기준(1차·2차·3차) 정렬이 필요할 때는 SORTBY를 씁니다.",
    "params": [
      {
        "name": "배열",
        "required": true,
    ...
    "examples": [
      ...
      },
      {
        ...
        "explain": "결과에는 이름만 나오지만 정렬은 청구액 기준. 기준 열을 결과에 포함하지 않아도 되는 게 SORT와의 핵심 차이입니다."
      },
      {
        "level": "advanced",
      ...
      },
      {
        ...
        "explain": "(기준배열, 정렬방향) 짝을 계속 이어 붙이면 1차·2차·3차 정렬이 됩니다. SORT의 단일 열 기준보다 다중 기준을 다루기 쉽습니다."
      },
      {
        "level": "advanced",
        ...
        "explain": "기준 자리에 표에 없는 계산식을 바로 넣을 수 있습니다. 별도 도우미 열 없이 손해율순 정렬이 됩니다 — SORT로는 도우미 열이 필요합니다."
      }
    ],
    "tips": "표 안의 열 번호 하나로 간단히 정렬할 때는 SORT, 결과에 없는 다른 범위·계산식을 기준으로 삼거나 1차·2차·3차 다중 기준 정렬이 필요할 때는 SORTBY를 씁니다. 다중 열 정렬은 SORT보다 SORTBY가 명확합니다. SORTBY의 기준배열은 결과에 표시되지 않아도 되지만 정렬할 배열과 '개수'는 반드시 같아야 하며, 다르면 #VALUE! 오류가 납니다. 두 함수 모두 원본 표의 순서는 바뀌지 않고 별도 위치에 정렬본이 새로 생기며, 결과가 펼쳐질 공간이 막혀 있으면 #SPILL! 오류가 납니다.",
    "related": [
      "FILTER",
      "UNIQUE",
      "LARGE",
  ...
  },
  {
    ...
    "intro": "VSTACK과 HSTACK은 흩어져 있는 여러 표나 목록을 수식 하나로 이어 붙여 주는 '쌓기(Stack)' 짝 함수입니다. VSTACK은 '세로로 쌓기(Vertical Stack)'로 여러 표를 위아래로 이어 붙이고, HSTACK은 '가로로 쌓기(Horizontal Stack)'로 여러 범위를 좌우로 나란히 붙입니다. 예를 들어 1월과 2월 계약 목록을 하나의 긴 목록으로 합칠 때는 VSTACK, 서로 떨어진 열들을 옆으로 붙여 새 표를 만들 때는 HSTACK을 씁니다.\n\n둘의 차이는 '붙이는 방향' 하나뿐입니다. 데이터를 아래로 늘리고 싶으면(행이 늘어남) VSTACK, 옆으로 늘리고 싶으면(열이 늘어남) HSTACK이라고 기억하면 됩니다. 두 함수 모두 결과가 '스필(spill)' 방식으로 자동으로 흘러넘쳐 표시되므로 한 칸에만 입력하면 되고, 원본이 바뀌면 합쳐진 결과도 자동으로 갱신됩니다. 복사·붙여넣기로 데이터를 이어 붙이던 수작업을 대체할 수 있습니다.\n\n주의할 점도 방향에 따라 짝을 이룹니다. VSTACK은 위아래로 쌓으므로 표들의 '열 개수'가 서로 달라야 문제가 생기고, HSTACK은 좌우로 붙이므로 '행 개수'가 다르면 문제가 됩니다. 어느 쪽이든 부족한 칸은 #N/A 오류로 채워지니, 붙이는 방향에 맞춰 열 수(VSTACK)나 행 수(HSTACK)를 맞춰 주세요. 두 함수는 서로 조합해 머리글·본문을 자유롭게 붙여 표를 통째로 조립하는 데 자주 함께 쓰입니다.",
    "params": [
      {
        "name": "배열1",
        "required": true,
    ...
    "examples": [
      ...
      },
      {
        ...
        "explain": "서로 떨어져 있던 두 열을 옆으로 붙여 한 표로 만듭니다. 원본에서 필요한 열만 골라 새 표를 구성할 때 편리합니다."
      },
      {
        "level": "basic",
      ...
      },
      {
        ...
        "explain": "HSTACK으로 열을 옆으로 붙이고, VSTACK으로 머리글을 위에 올려 표를 통째로 만듭니다. 두 함수를 조합하면 원하는 모양의 표를 자유롭게 구성할 수 있습니다."
      },
      {
        "level": "advanced",
    ...
    "tips": "붙이는 방향으로 선택하세요. 데이터를 아래로 늘리려면(행 추가) VSTACK, 옆으로 늘리려면(열 추가) HSTACK입니다. 크기가 안 맞으면 부족한 칸이 #N/A로 채워지는데, VSTACK은 표들의 '열 개수', HSTACK은 '행 개수'를 맞춰야 합니다(필요하면 IFERROR로 감싸 처리). 두 함수 모두 최대 255개 배열까지 이어 붙일 수 있고, 서로 조합하면 머리글·본문을 붙여 표를 통째로 조립할 수 있습니다.",
    "related": [
      "TOCOL",
      "TOROW",
      "CHOOSECOLS",
  ...
  },
  {
    ...
    "intro": "TAKE와 DROP은 표에서 '처음 몇 줄' 또는 '마지막 몇 줄'을 다루는 짝 함수입니다. TAKE는 지정한 부분을 '가져오고(남기고)', DROP은 지정한 부분을 '떼어내고(버리고)' 나머지를 돌려줍니다. 서로 정반대라고 생각하면 됩니다.\n\n두 함수 모두 행 개수를 양수로 주면 위(처음)에서부터, 음수로 주면 아래(끝)에서부터 셉니다. 세 번째 인수(열수)도 같은 방식이며, 양수는 왼쪽에서·음수는 오른쪽에서입니다. 두 번째 인수를 쉼표로 비우면 행은 건드리지 않고 열만 다룰 수 있습니다.\n\n언제 무엇을 쓸까요? '정렬된 목록에서 상위 10건만 남기고 싶다'처럼 원하는 부분만 남길 때는 TAKE, '제목 행(머리글)을 떼어내고 데이터만 쓰고 싶다'처럼 필요 없는 부분을 버릴 때는 DROP이 자연스럽습니다. 둘을 겹쳐 쓰면 =TAKE(DROP(범위,10),10)처럼 '11~20번째 행'같은 표의 중간 구간만 잘라낼 수도 있습니다. 필터·정렬 기능과 달리 수식이라 원본이 바뀌면 결과도 자동으로 갱신됩니다.",
    "params": [
      {
        "name": "배열",
        "required": true,
      ...
      },
      {
        "name": "행수",
        "required": true,
        "desc": "다룰 행 수. 양수=위(처음)에서부터, 음수=아래(끝)에서부터. TAKE는 이만큼 '가져오고', DROP은 이만큼 '제거'합니다(DROP으로 제목 행 제거는 1). 열만 지정하려면 쉼표로 자리를 비웁니다."
      },
      {
        "name": "열수",
        "required": false,
    ...
    "examples": [
      ...
      },
      {
        ...
        "explain": "DROP으로 앞 10줄을 버리고 TAKE로 그다음 10줄만 가져옵니다. 페이지 나누기(11~20위 구간)처럼 표의 중간만 잘라낼 때 쓰는 대표 조합입니다."
      }
    ],
    "tips": "둘은 정반대입니다 — 원하는 부분을 '남길 때'는 TAKE, '버릴 때'는 DROP을 쓰세요. 공통 규칙: 양수=처음에서부터, 음수=끝에서부터이며, 두 번째 인수를 쉼표로 비우면 열만 다룹니다(예: =TAKE(범위,,2)). TAKE는 요청한 개수가 실제 크기보다 크면 있는 만큼만 돌려주지만, DROP은 제거할 개수가 전체 행/열 수보다 크거나 같으면 남는 것이 없어 #CALC! 오류가 납니다. 둘을 조합하면 표의 중간 구간만 잘라낼 수 있습니다.",
    "related": [
      "CHOOSEROWS",
      "CHOOSECOLS",
      "SORT",
  ...
  },
  {
    ...
    "intro": "COUNTIF와 COUNTIFS는 둘 다 '조건에 맞는 셀이 몇 개인지' 세는 함수입니다. 값을 더하는 SUMIF/SUMIFS의 '개수 세기' 버전이라고 보면 됩니다. 예를 들어 상품이 '자동차'인 계약이 몇 건인지, 청구액이 100만 원 이상인 건이 몇 개인지 한 줄로 셀 수 있습니다.\n\n둘의 차이는 '조건의 개수'입니다. 조건이 하나뿐이면 COUNTIF(단수)를, 조건이 둘 이상이면 COUNTIFS(복수, 끝에 S)를 씁니다. COUNTIFS에 조건을 여러 개 적으면 그 조건들을 전부 만족하는 셀만 세므로 '그리고(AND)' 개념이 됩니다. 예를 들어 '자동차 상품이면서 보험료가 100만 원을 넘는 계약이 몇 건일까?'는 COUNTIFS로 답합니다. (조건이 하나뿐인 COUNTIFS는 COUNTIF와 결과가 같으니, 처음부터 COUNTIFS로 통일해 두면 조건을 추가할 때 함수 이름을 바꿀 필요가 없습니다.)\n\n두 함수 모두 조건은 값(\"자동차\"), 크기 비교(\">=1000000\"), 이름의 일부(와일드카드 \"김*\", \"*상해*\")로 줄 수 있습니다. 비교나 별표를 쓸 때는 반드시 큰따옴표로 감싸고, 셀 값을 넣을 때는 연산자만 따옴표로 묶고 셀은 &로 잇습니다(\">=\"&F1). 실무에서는 단순 집계뿐 아니라 COUNTIF로 '이 값이 명단에 있는가(존재 확인)'·'중복이 있는가'를 점검하고, COUNTIFS로 기간+분류 같은 다중 필터 건수를 집계하는 데 많이 씁니다.",
    "params": [
      {
        "name": "범위 / 조건범위1",
        "required": true,
    ...
    "examples": [
      {
        ...
        "explain": "조건 하나에 맞는 셀이 몇 개인지 센다. 상품이 자동차인 계약이 몇 건인지 바로 알 수 있다."
      },
      {
        "level": "basic",
        ...
        "explain": "조건을 (범위, 기준) 짝으로 나란히 적으면 둘 다 만족하는 건수만 센다(AND). 크기 비교는 \">1000000\"처럼 부등호와 숫자를 큰따옴표 안에 함께 넣는다."
      },
      {
        "level": "basic",
      ...
      },
      {
        ...
        "explain": "같은 열(B)을 두 번 써서 '이상'과 '이하'를 걸면 기간 필터가 된다. 셀 F1 값을 조건에 넣을 때는 부등호 문자열과 셀을 &로 이어 붙인다(\">=\"&F1). F1, F2만 바꾸면 기간을 바로 바꿀 수 있다."
      },
      {
        "level": "advanced",
        ...
        "explain": "H2에 =UNIQUE(C2:C100) 같은 스필 수식이 있으면 H2#는 그 결과 전체를 가리킨다. 조건 자리에 H2#를 넣으면 상품 종류마다 건수를 한 번에 계산해 표가 스스로 늘고 준다(365 신규 동작). 부분일치가 필요하면 조건에 \"*생명*\"처럼 별표를 써서 '생명'이 포함된 모든 상품을 셀 수도 있다."
      },
      {
        "level": "advanced",
    ...
    "tips": "조건이 하나면 COUNTIF, 둘 이상이면 COUNTIFS(끝에 S)를 쓴다. COUNTIFS는 처음부터 써 두면 조건을 추가할 때 함수 이름을 바꿀 필요가 없어 편하다. 두 함수 모두 대소문자를 구분하지 않고 와일드카드 *(임의 문자열)·?(임의 한 글자)를 쓸 수 있으며, 실제 * 나 ? 글자를 찾으려면 앞에 물결(~)을 붙여 ~*, ~? 로 쓴다. 숫자 크기 비교는 \">=60\"처럼 따옴표 안에, 셀 참조와 함께 쓸 때는 \">=\"&F1 로 연산자만 따옴표로 묶고 셀은 &로 잇는다. 빈 셀은 \"\", 비어있지 않은 셀은 \"<>\"로 센다. COUNTIFS의 모든 조건범위는 행 수(크기)가 같아야 하며, 다르면 #VALUE! 오류가 난다.",
    "related": [
      "SUMIF",
      "SUMIFS",
      "AVERAGEIFS",
  ...
  },
  {
    ...
    "intro": "SUMIF와 SUMIFS는 둘 다 '조건에 맞는 값만 골라서 더하는' 함수입니다. 전부 더하는 SUM과 달리, 먼저 조건을 검사해 통과한 행의 숫자만 합산합니다. 예를 들어 상품명이 적힌 열에서 '자동차'인 행만 찾아 그 보험료를 더할 수 있습니다.\n\n둘의 차이는 '조건의 개수'입니다. SUMIF는 조건을 하나만 걸 때 쓰고, SUMIFS는 여러 조건을 겹쳐서 모두 만족하는 값만 더할 때 씁니다. 예컨대 '상품이 자동차'만 보면 SUMIF, '상품이 자동차이면서 지역이 서울'처럼 조건이 둘 이상이면 SUMIFS입니다. SUMIFS에 건 조건들은 전부 AND로 이어져 하나라도 안 맞으면 제외됩니다. 그래서 '조건 1개=SUMIF, 2개 이상=SUMIFS'로 기억하면 쉽습니다.\n\n가장 헷갈리는 점은 인수 순서입니다. SUMIF는 '조건범위'가 맨 앞에 오고 더할 범위(합계범위)는 맨 뒤(생략 가능)입니다. 반대로 SUMIFS는 '더할 범위(합계범위)'가 맨 앞에 오고 그 뒤로 조건범위·조건이 쌍으로 이어집니다. 조건을 검사하는 열과 더하는 열이 같으면 SUMIF에서는 합계범위를 생략할 수 있습니다. 조건은 값('자동차'), 크기 비교('>=1000000'), 이름의 일부(와일드카드 '*상해*') 등으로 자유롭게 줄 수 있고, SUMIFS는 이런 조건 쌍을 최대 127쌍까지 넣을 수 있습니다.",
    "params": [
      {
        "name": "합계범위",
        "required": true,
        "desc": "실제로 더할 값이 든 범위. SUMIF에서는 맨 뒤 인수이며 생략하면 조건범위 자체를 더한다(선택). SUMIFS에서는 맨 앞에 오는 필수 인수다. 모든 조건범위와 크기·모양이 같아야 한다."
      },
      {
        "name": "조건범위(조건범위1)",
        "required": true,
    ...
    "examples": [
      {
        ...
        "explain": "B열에서 \"자동차\"를 찾아 같은 행의 D열 값만 더한다. 조건을 검사하는 범위(B)가 맨 앞, 실제로 더하는 범위(D)가 맨 뒤에 온다. 조건이 하나뿐이므로 SUMIF를 쓴다."
      },
      {
        "level": "basic",
        ...
        "explain": "조건을 검사하는 열과 더할 열이 같으면 세 번째 인수를 생략한다. 여기서는 D열에서 100만 이상인 값만 골라 더한다. 이 생략은 SUMIF에서만 가능하다."
      },
      {
        "level": "basic",
        ...
        "explain": "조건이 두 개이므로 SUMIF 대신 SUMIFS를 쓴다. 더할 범위(E)를 맨 앞에 두고 뒤에 조건범위·조건을 쌍으로 이어 붙인다. 두 조건은 AND로 이어져 둘 다 맞아야 합산된다."
      },
      {
        "level": "advanced",
        ...
        "explain": "비교연산자 \">=\"와 셀 값을 &로 이어 붙이면 기준을 셀로 바꿀 수 있다. F1 숫자만 고치면 합계가 자동으로 다시 계산된다. 같은 기법을 SUMIFS의 조건 자리에도 그대로 쓸 수 있다."
      },
      {
        "level": "advanced",
        ...
        "explain": "*는 임의의 문자열을 뜻한다. 상품명이 정확히 일치하지 않아도 \"상해\"라는 글자를 포함하면 모두 더한다. 조건이 하나이므로 SUMIF로 충분하다."
      },
      {
        "level": "advanced",
        ...
        "explain": "같은 날짜 범위(A열)에 '이상(>=)'과 '이하(<=)' 두 조건을 걸면 '구간 사이'가 된다. 조건이 둘이므로 SUMIFS를 쓴다. 1분기 청구액 합계를 구하는 전형적인 방법이다."
      },
      {
        "level": "advanced",
        ...
        "explain": "행 기준($G2)과 열 기준(H$1)의 참조 고정을 다르게 주면, 수식 하나를 표 전체에 복사해 상품×지역 교차 합계표를 만들 수 있다. SUMIF로 단일 조건 요약표를 만들 때는 =SUMIF($B$2:$B$100, F2, $D$2:$D$100)처럼 조건을 하나만 둔다."
      },
      {
        "level": "advanced",
    ...
    "tips": "조건이 하나면 SUMIF, 둘 이상이면 SUMIFS를 쓴다. 가장 흔한 실수는 인수 순서다 — SUMIF는 조건범위가 맨 앞, SUMIFS는 합계범위가 맨 앞에 온다. 두 함수 모두 조건범위(들)와 합계범위는 크기·모양이 같아야 한다(같은 행끼리 대응). 텍스트 조건은 \"\" 안에 넣고, 비교연산자와 셀 참조를 함께 쓸 때는 \">=\"&A1처럼 &로 이어 붙인다. 대소문자는 구분하지 않는다. SUMIFS의 조건들은 AND(모두 만족)로만 묶이므로, OR로 합치려면 SUMIFS 여러 개를 더하거나 SUMPRODUCT를 쓴다. '구간 사이' 합계는 같은 범위에 >= 와 <= 두 조건을 준다.",
    "related": [
      "COUNTIF",
      "AVERAGEIF",
      "COUNTIFS",
  ...
  },
  {
    ...
    "intro": "REDUCE와 SCAN은 배열의 값을 왼쪽(위)부터 하나씩 순서대로 훑으면서 '누적값'을 계속 갱신해 가는 형제 함수입니다. 눈덩이를 굴리듯, 앞까지의 결과(누적값)에 지금 값(현재값)을 합쳐 다음 누적값을 만드는 방식으로 동작합니다.\n\n둘 다 세 부분으로 이해합니다. ① 초기값 — 누적을 시작할 첫 값(합계면 0, 곱이면 1, 문자열 이어붙이기면 \"\"). ② 배열 — 훑어 갈 값들. ③ LAMBDA(누적값, 현재값, 계산식) — 지금까지의 누적값과 이번 값을 어떻게 합칠지 정하는 규칙입니다. 엑셀은 배열의 각 값마다 이 LAMBDA를 실행하며 누적값을 갱신합니다.\n\n핵심 차이는 '무엇을 돌려주느냐'입니다. REDUCE는 다 훑고 난 뒤 마지막 누적값 하나만 돌려줍니다(줄이기·reduce). SCAN은 매 단계의 누적값을 원본과 같은 크기의 배열로 전부 돌려줍니다(달리는 합계·running total). 예를 들어 청구액 10, 20, 30을 더하면 REDUCE는 60 하나를, SCAN은 10, 30, 60을 모두 보여 줍니다.\n\n그래서 선택 기준은 간단합니다. 최종 합계·곱·집계처럼 결과 하나만 필요하면 REDUCE, 각 시점까지의 누계·이력처럼 중간 과정 전체가 필요하면 SCAN을 씁니다. 단순 합·곱은 SUM으로 충분하지만, '조건에 따라 다르게 누적하기'나 '배열을 점점 쌓아 가기'처럼 일반 함수로 어려운 반복 로직을 표현할 때 이 둘이 강력합니다. 개념이 다소 추상적이라 난이도가 높은 편입니다.",
    "params": [
      {
        "name": "초기값 / 초깃값",
        "required": false,
        "desc": "누적을 시작할 첫 값. 합계는 0, 곱(복리)은 1, 문자열 이어붙이기는 \"\"처럼 계산에 맞게 정합니다. REDUCE는 생략 가능(비우면 빈 값 0에서 시작)하지만, SCAN은 반드시 지정해야 합니다."
      },
      {
        "name": "배열",
        "required": true,
      ...
      },
      {
        ...
        "required": true,
        "desc": "누적 규칙. 첫 매개변수는 지금까지의 누적값(REDUCE: 누적값 / SCAN: 누계), 둘째는 이번에 처리할 현재값이며, 계산식의 결과가 다음 누적값이 됩니다. 이름은 자유롭게 지어도 되지만 '순서(누적값 먼저, 현재값 나중)'는 지켜야 합니다."
      }
    ],
    "examples": [
      {
        ...
        "result": "10",
        "explain": "초기값 0에서 시작해 값을 하나씩 더해 갑니다: 0→1→3→6→10. 마지막 누적값 10 '하나'만 남는 것이 REDUCE의 핵심입니다(과정은 보이지 않음)."
      },
      {
        "level": "basic",
        ...
        "explain": "0에서 시작해 10 → 30 → 60으로 더해 가되, 각 단계의 누적값이 그대로 배열로 나옵니다. 같은 덧셈이라도 REDUCE는 60 하나, SCAN은 과정 전체(10,30,60)를 돌려준다는 차이를 바로 보여 줍니다."
      },
      {
        "level": "advanced",
        ...
        "explain": "누적 규칙 안에 IF를 넣어 조건을 만족하는 값만 더합니다. SUMIF로도 되지만, REDUCE는 이렇게 '누적하면서 판단'하는 복잡한 로직을 자유롭게 표현하고 결과 하나로 집계할 수 있음을 보여 줍니다."
      },
      {
        "level": "advanced",
        ...
        "explain": "E열의 연도별 수익률에 1+수익률을 곱해 나가면 '원금 대비 누적 성장배수'가 각 연도마다 나옵니다. 곱셈이라 초깃값을 1로 둡니다. 매 시점의 누적 배수(과정)가 필요하니 SCAN이 제격입니다."
      },
      {
        "level": "advanced",
        ...
        "explain": "같은 러닝 토탈이라도, SCAN은 한 줄이면 끝납니다. REDUCE로도 만들 수는 있지만 VSTACK으로 직접 배열을 쌓고 시작용 0을 DROP으로 떼어 내야 하므로 훨씬 번거롭습니다. '중간 과정을 배열로 남기고 싶으면 SCAN', '마지막 상태 하나만 필요하면 REDUCE'라는 선택 기준을 명확히 보여 줍니다."
      }
    ],
    "tips": "선택 기준: 최종 결과 하나만 필요하면 REDUCE, 각 단계의 누적 '과정' 전체가 필요하면 SCAN을 씁니다(러닝 토탈·누적 최고값·진행 이력 등). 초기값은 계산 종류에 맞춰야 합니다 — 합계 0, 곱 1, 문자열 이어붙이기 \"\". REDUCE는 초기값을 생략할 수 있지만 SCAN은 필수입니다. LAMBDA의 인수 이름은 자유롭게 지어도 되나 순서(누적값 먼저, 현재값 나중)는 지켜야 합니다. SCAN은 원본과 같은 크기로 스필되므로 아래(옆) 칸이 비어 있어야 #SPILL! 오류가 나지 않습니다. 값마다 독립적으로 변환만 할 때는 누적이 필요 없으니 MAP이 맞습니다.",
    "related": [
      "MAP",
      "LAMBDA",
      "VSTACK",
  ...
  },
  {
    ...
    "intro": "구조적 참조는 셀 주소(D2:D100) 대신 '표 이름과 열 이름'으로 데이터를 가리키는 방법입니다. 먼저 데이터를 표로 만들어야 합니다(범위 선택 후 Ctrl+T). 그러면 표에 이름(예: 표1, 계약)이 생기고, 열 제목으로 그 열 전체를 부를 수 있습니다.\n\n대괄호 [ ] 안의 특별한 항목이 핵심입니다. 열 이름(계약[보험료])은 그 열의 '데이터 부분 전체'를 뜻합니다. 그리고 # 로 시작하는 항목은 영역을 가리킵니다 — [#All]은 제목+데이터+합계 전체, [#Data]는 데이터만, [#Headers]는 제목 줄, [#Totals]는 합계 줄입니다. 배열 영역을 이렇게 # 로 간단히 지정합니다.\n\n@ 는 '현재 행'을 뜻합니다. 표 안에서 =[@보험료]*0.1 처럼 쓰면 '이 행의 보험료'를 가리켜, 행마다 자동으로 자기 행 값을 계산합니다(계산 열).\n\n가장 큰 장점은 자동 확장입니다. 표에 행을 추가하면 계약[보험료] 같은 참조 범위가 자동으로 늘어나므로, SUM·SUMIFS 등의 수식을 고칠 필요가 없습니다. 이름으로 읽혀 수식의 뜻도 분명해집니다.",
    "params": [
      {
        "name": "표이름[열이름]",
        "required": true,
    ...
    "examples": [
      {
        ...
        "explain": "셀 주소 대신 표이름[열이름]으로 그 열 데이터 전체를 참조합니다. 표에 행을 추가해도 범위가 자동으로 늘어 합계가 갱신됩니다."
      },
      {
        "level": "basic",
        ...
        "explain": "표 안에서 @는 '이 행'을 뜻합니다. 한 셀에 입력하면 열 전체에 자동으로 채워져, 행마다 자기 행 보험료로 계산합니다."
      },
      {
        "level": "advanced",
        ...
        "explain": "SUMIFS 같은 함수의 범위 자리에 표 열을 넣으면 수식이 읽기 쉽고, 데이터가 늘어도 자동으로 반영됩니다. 셀 주소 고정($)이 필요 없습니다."
      },
      {
        "level": "advanced",
        ...
        "explain": "[#All]은 제목·데이터·합계를 모두 포함한 표 전체 영역입니다. [#Data]는 데이터만, [#Headers]는 제목 줄만 가리킵니다 — 배열 영역을 # 항목으로 간단히 지정합니다."
      }
    ],
    "tips": "먼저 데이터를 표로 만들어야 합니다(Ctrl+T). 표 이름은 [표 디자인] 탭에서 바꿀 수 있습니다. #은 영역(#All·#Data·#Headers·#Totals), @는 현재 행을 뜻합니다. 열 이름에 공백·특수문자가 있으면 [@[열 이름]]처럼 안쪽 대괄호로 감쌉니다.",
    "related": [
      "스필 범위 연산자 (#)",
      "SUMIFS",
      "XLOOKUP",
  ...
  },
  {
    ...
    "intro": "트림 참조는 범위 참조의 콜론(:) 바로 옆에 마침표(.)를 붙여, 범위 가장자리의 빈 셀을 자동으로 잘라 내는 새 기능입니다(Excel 2024·Microsoft 365).\n\n점을 찍는 위치가 '자를 쪽'입니다. 콜론 뒤에 찍으면(A2:.A100) 아래쪽(후행) 빈칸을 자르고, 콜론 앞에 찍으면(A2.:A100) 위쪽(선행) 빈칸을, 양쪽에 찍으면(A2.:.A100) 앞뒤 모두 자릅니다. 후행 트림(:.)이 가장 많이 쓰입니다.\n\n가장 강력한 활용은 전체 열 참조와의 결합입니다. A:.A 처럼 쓰면 'A열에서 데이터가 있는 마지막 행까지만' 자동으로 참조합니다. 데이터가 늘거나 줄면 범위도 따라 조정되므로, 수식을 고치지 않아도 항상 데이터에 딱 맞습니다. 백만 개의 빈 셀을 끌어와 느려지거나 0·빈칸이 잔뜩 나오던 고전적인 문제를 해결합니다.\n\n함수형 TRIMRANGE(범위, [행_방식], [열_방식])도 같은 일을 합니다. 방식 인수는 0=자르지 않음, 1=앞만, 2=뒤만, 3=양쪽입니다(기본 3). 점(.) 연산자는 이 함수를 짧게 부르는 축약형이라고 보면 됩니다.",
    "params": [
      {
        "name": ":. (콜론 뒤 점)",
        "required": false,
    ...
    "examples": [
      {
        ...
        "explain": "콜론 뒤 점(:.)이 아래쪽 빈칸을 잘라 냅니다. 범위를 넉넉히 D100까지 잡아도 실제 데이터까지만 계산합니다."
      },
      {
        "level": "basic",
        ...
        "explain": "전체 열 참조 D:D 대신 D:.D 를 쓰면 데이터 끝까지만 봅니다. 행이 추가돼도 범위가 자동으로 늘어 수식을 고칠 필요가 없습니다."
      },
      {
        "level": "advanced",
        ...
        "explain": "FILTER·SORT 같은 동적 배열 함수의 입력에 트림 참조를 쓰면, 데이터가 바뀌어도 항상 채워진 부분만 처리합니다. 스필과 궁합이 좋습니다."
      },
      {
        "level": "advanced",
        ...
        "explain": "점 연산자 대신 함수로도 같은 일을 합니다. 두 번째 인수 3은 '앞뒤 모두 자르기'입니다(1=앞, 2=뒤). 다른 함수의 인수로 넣어 재사용하기 좋습니다."
      }
    ],
    "tips": "Excel 2024·Microsoft 365 전용입니다(2021 이하 미지원). 점은 '자를 쪽'에 찍습니다 — 콜론 뒤(:.)=후행, 콜론 앞(.:)=선행. 전체 열과 결합한 A:.A 가 가장 유용합니다. 완전히 빈 가장자리 행·열을 잘라 내며, 중간의 빈칸은 남깁니다.",
    "related": [
      "스필 범위 연산자 (#)",
      "FILTER",
      "SORT",
