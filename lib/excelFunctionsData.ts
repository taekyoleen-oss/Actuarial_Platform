...
export const EXCEL_FUNCTIONS: ExcelFunction[] = [
  ...
  },
  {
    ...
    "intro": "QUARTILE.INC와 QUARTILE.EXC는 둘 다 데이터를 작은 값부터 크기순으로 줄 세운 뒤, 4등분하는 위치의 값(사분위수)을 알려주는 함수입니다. 청구액이나 성적을 늘어놓고 '아래에서 25% 지점은 얼마인가?', '한가운데(50%) 값은?' 같은 질문에 답해 줍니다. 두 번째 인수로 위치를 고르고, 정확히 그 위치에 값이 없으면 양옆 값을 비례로 섞어(보간) 계산하는 것도 똑같습니다.\n\n차이는 양 끝(최솟값·최댓값)을 계산에 넣느냐입니다. '.INC'는 Inclusive(포함)의 약자로, 0%(최솟값)와 100%(최댓값)까지 포함하는 전통적 방식입니다. 그래서 두 번째 인수에 0~4를 넣을 수 있습니다(0=최솟값, 1=25%, 2=중앙값, 3=75%, 4=최댓값). '.EXC'는 Exclusive(제외)의 약자로, 양 끝을 빼고 그 사이만 나눕니다. 그래서 인수는 1(25%)·2(50%)·3(75%)만 되고, 0이나 4를 넣으면 #NUM! 오류가 납니다.\n\n계산 위치를 EXC는 데이터 개수 n이 아니라 (n+1) 기준으로 잡기 때문에, 같은 데이터라도 INC보다 바깥쪽(더 극단적인) 값이 나오는 경향이 있습니다. 어느 쪽이 '정답'이라기보다, INC는 최솟·최댓값까지 필요한 상자수염그림·IQR 계산에 편하고, EXC는 표본으로 전체 집단을 좀 더 보수적으로 추정할 때 씁니다. 중요한 건 한 보고서 안에서 방식을 섞지 말고 하나로 통일하는 것입니다.",
    "params": [
      {
        "name": "배열",
        "required": true,
      ...
      },
      {
        "name": "사분위수",
        "required": true,
        "desc": "어느 위치를 구할지 지정합니다. INC는 0~4(0=최솟값, 1=25%, 2=50%(중앙값), 3=75%, 4=최댓값), EXC는 1~3(1=25%, 2=50%, 3=75%)만 가능하며 EXC에 0·4를 넣으면 오류가 납니다."
      }
    ],
    "examples": [
      {
        ...
        "explain": "청구액을 작은 값부터 줄 세웠을 때 아래에서 4분의 1 지점 값입니다. 두 번째 인수 1이 '1사분위(25%)'를 뜻하며, INC는 양 끝을 포함하는 전통적 방식입니다."
      },
      {
        "level": "basic",
      ...
      },
      {
        ...
        "explain": "양 끝(최솟·최댓값)을 제외하고 (n+1) 기준으로 위치를 잡아 1사분위를 구합니다. 같은 데이터라도 QUARTILE.INC보다 더 극단적인 값이 나오는 경향이 있습니다."
      },
      {
        "level": "advanced",
      ...
      },
      {
        ...
        "explain": "같은 데이터라도 양 끝 포함(INC)·제외(EXC)로 계산이 달라 값이 벌어집니다. 그 차이를 직접 확인해 어느 방식을 보고서에 쓸지 판단할 때 씁니다."
      },
      {
        "level": "advanced",
        ...
        "explain": "EXC는 최솟값(0)·최댓값(4)을 지원하지 않아 오류가 납니다. IFERROR로 감싸 오류 대신 이유를 표시하면 표가 깨지지 않고 깔끔해집니다. 최솟·최댓값이 필요하면 INC(0·4)나 MIN·MAX를 쓰세요."
      },
      {
        "level": "advanced",
        ...
        "explain": "SEQUENCE로 0~4를 만들어 넣으면 다섯 사분위수가 한 수식으로 아래로 펼쳐집니다(스필, 2021 이상). EXC로 같은 걸 하려면 0·4를 못 쓰므로 SEQUENCE(3,1,1,1)로 25·50·75%만 뽑습니다."
      }
    ],
    "tips": "두 함수의 갈림길은 양 끝 포함 여부입니다. INC는 인수 0=MIN, 2=MEDIAN, 4=MAX와 결과가 같고 최솟·최댓값까지 필요한 상자수염그림·IQR에 편합니다. EXC는 1·2·3만 되고 (n+1) 기준이라 더 바깥쪽 값이 나와, 표본으로 모집단을 보수적으로 추정할 때 씁니다. 공통 주의: 두 함수 모두 '위에서 몇 %'가 아니라 '아래에서(작은 값부터) 몇 %'를 지정하므로, 상위 25% 합격선은 인수 3(하위 75% 경계)을 넣어야 합니다. 범위를 벗어난 인수(INC는 0~4 밖, EXC는 0·4)나 데이터가 너무 적으면 #NUM! 오류가 납니다. INC와 EXC는 결과가 다르니 한 보고서 안에서 섞지 말고 방식을 통일하세요.",
    "related": [
      "PERCENTILE.INC",
      "PERCENTILE.EXC",
      "MEDIAN",
  ...
  },
  {
    ...
    "intro": "VLOOKUP과 HLOOKUP은 '표에서 값을 찾아오는' 짝꿍 함수입니다. 두 함수 모두 표에서 기준값을 먼저 찾은 뒤, 그와 연결된 다른 값을 가져옵니다. 이름의 V는 세로(Vertical), H는 가로(Horizontal)를 뜻하며, 이 방향 차이가 둘의 유일한 핵심 차이입니다.\n\nVLOOKUP은 표의 '맨 왼쪽 열'에서 값을 찾아, 같은 행의 오른쪽 어느 열에 있는 값을 가져옵니다. 예: 상품 목록표에서 상품코드를 찾아 그 상품의 보험료를 가져올 때. 데이터가 위에서 아래로 쌓이는(세로) 표에 씁니다. 실무 데이터는 대부분 이렇게 세로로 정리되므로, 둘 중 훨씬 자주 쓰이는 쪽이 VLOOKUP입니다.\n\nHLOOKUP은 VLOOKUP을 90도 돌린 '가로 버전'입니다. 표의 '맨 윗 행'에서 값을 찾아, 같은 열의 아래쪽에 있는 값을 가져옵니다. 예: 1행에 1월·2월·3월…이 가로로 나열되고 그 아래로 월별 값이 있는 요약표에서 특정 월의 값을 뽑을 때. 데이터가 가로로 나열된 표에 씁니다.\n\n정리하면, 찾는 기준이 '왼쪽 세로 열'에 있으면 VLOOKUP, '맨 윗 가로 행'에 있으면 HLOOKUP을 쓰면 됩니다. 나머지 사용법(인수 순서·마지막 일치옵션)은 완전히 같고, VLOOKUP은 '열 번호', HLOOKUP은 '행 번호'를 지정한다는 점만 다릅니다. 두 함수 모두 마지막 인수를 FALSE로 두면 '정확히 일치', TRUE(또는 생략)로 두면 '근삿값'을 찾으며, 초보자는 거의 항상 FALSE(정확히 일치)를 쓰면 됩니다.",
    "params": [
      {
        "name": "찾을값",
        "required": true,
      ...
      },
      {
        "name": "표범위",
        "required": true,
        "desc": "검색할 표 전체 범위입니다. VLOOKUP은 이 범위의 첫 번째(맨 왼쪽) 열에, HLOOKUP은 첫 번째(맨 위) 행에 찾을값이 있어야 합니다. 예: $E$2:$G$5, B1:F5."
      },
      {
        "name": "열번호 / 행번호",
        "required": true,
        "desc": "가져올 값의 위치를 숫자로 지정합니다. VLOOKUP은 '열번호'(왼쪽 열이 1, 예: 3=세 번째 열), HLOOKUP은 '행번호'(맨 윗 행이 1, 예: 2=두 번째 행)를 씁니다. 두 함수의 유일한 인수 차이입니다."
      },
      {
        "name": "일치옵션",
        "required": false,
        "desc": "두 함수 공통. FALSE(또는 0)=정확히 일치, TRUE(또는 1·생략)=근삿값. 근삿값은 기준(VLOOKUP은 첫 열, HLOOKUP은 첫 행)이 오름차순 정렬되어야 합니다. 보통 FALSE를 씁니다."
      }
    ],
    "examples": [
      {
      ...
      },
      {
        ...
        "explain": "표의 맨 윗 행(B1:F1)에 상품명이 가로로 나열되어 있고 그 아래 2행에 보험료가 있을 때, \"암보험\" 열을 찾아 2번째 행 값을 가져옵니다. VLOOKUP과 방향(세로↔가로)만 다를 뿐 원리는 같습니다. 여기서 2는 '행번호'라는 점에 주의하세요."
      },
      {
        "level": "advanced",
        ...
        "explain": "VLOOKUP·HLOOKUP은 찾는 값이 없으면 #N/A 오류를 냅니다. IFERROR로 감싸면 오류 대신 원하는 문구를 보여줄 수 있어, 보고서에 오류 표시가 그대로 노출되는 것을 막습니다(HLOOKUP도 같은 방식으로 감쌉니다)."
      },
      {
        "level": "advanced",
        ...
        "explain": "행번호를 2처럼 고정하면 표에 행이 늘거나 순서가 바뀔 때 수식을 고쳐야 합니다. MATCH로 A열의 항목 이름 중 \"claim_amt\"가 몇 번째 행인지 찾아 넣으면, 표 구조가 바뀌어도 알아서 맞는 행을 가져옵니다. VLOOKUP에서는 같은 요령을 '열번호' 자리에 MATCH(\"premium\", 헤더행, 0)로 넣습니다."
      },
      {
        "level": "advanced",
        ...
        "explain": "찾을 조건이 두 가지(예: 지역코드 A3, 상품코드 B3)일 때, A3&B3로 두 값을 이어붙여 하나의 '조합키'로 만들어 찾습니다. 표의 첫 열에도 미리 같은 방식(=D3&E3)으로 조합키를 만들어 두어야 합니다. 여러 조건 검색이 잦다면 XLOOKUP·INDEX+MATCH가 더 편할 수 있습니다."
      }
    ],
    "tips": "방향으로 고른다: 기준값이 표의 '왼쪽 세로 열'에 있으면 VLOOKUP, '맨 윗 가로 행'에 있으면 HLOOKUP입니다. 실무 데이터는 대부분 세로로 쌓이므로 VLOOKUP이 훨씬 자주 쓰이고, HLOOKUP은 월별·분기별 가로 요약표에서 필요합니다. 흔한 실수: ① VLOOKUP은 가져올 값이 기준 열의 왼쪽에 있으면 못 가져옵니다(XLOOKUP 또는 INDEX+MATCH 사용). ② 마지막 인수를 생략하면 두 함수 모두 근삿값(TRUE)으로 동작해 엉뚱한 값이 나올 수 있으니, 정확히 찾을 땐 꼭 FALSE를 쓰세요. ③ 찾을값과 표의 값이 '숫자 vs 텍스트 숫자'로 형식이 다르면 못 찾습니다. 참고로 XLOOKUP은 검색모드에 따라 가로·세로를 하나로 처리할 수 있고, 가로↔세로 방향을 바꾸고 싶을 땐 TRANSPOSE가 도움이 됩니다.",
    "related": [
      "XLOOKUP",
      "INDEX",
      "MATCH",
