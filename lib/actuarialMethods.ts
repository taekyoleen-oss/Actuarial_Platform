...
export const ACTUARIAL_METHODS: StatMethod[] = [
  {
    ...
    params: [
      {
        name: "중앙노출 vs 초기노출",
        desc: "중앙노출(실제 관찰연수 합)은 중앙사망률 mx=D/Ec를, 초기노출(사망자는 그 보험연도 말까지 1년 인정)은 qx=D/Ei를 추정합니다 — 어느 율을 산출하느냐에 따라 노출 정의가 달라집니다.",
      },
      {
        name: "연령 기준(도달연령·밴딩)",
        desc: "노출은 관찰 시점의 도달연령(보험연령)에 배분합니다. 5세 구간 밴딩은 표본 확보와 연령 추세 표현 사이의 절충 — 셀을 잘게 쪼갤수록 사망 0건 셀이 속출합니다.",
      },
      {
        name: "alpha(신뢰수준)",
        desc: "정확 신뢰구간의 유의수준 — 0.05면 95% 구간. 하한 chi2.ppf(α/2, 2D)/(2E), 상한 chi2.ppf(1−α/2, 2(D+1))/(2E)의 닫힌형입니다.",
      },
      {
        name: "참조율(기대사망의 기준)",
        desc: "A/E 분모가 되는 기준율 — 국내 실무는 보험개발원 참조순보험요율·경험생명표가 표준. 노출과 율의 기준(초기노출↔qx, 중앙노출↔mx)을 반드시 일치시켜야 합니다.",
      },
      {
        name: "D=0(무사망 셀) 처리",
        desc: "하한은 0, 상한은 chi2.ppf(1−α/2, 2)/(2E) — '한 건도 못 봤다'는 것도 정보이므로 상한이 그 한계를 담습니다(정규근사는 0±0으로 무너짐).",
      },
      {
        name: "UDD(연내 균등사망 가정)",
    ...
    intro:
      "위험률 산출의 첫 단계는 '누가 얼마나 오래 위험에 노출되었는가'를 세는 것입니다. 계약별 관찰기간을 합한 것이 노출(exposure), 사망수를 노출로 나눈 것이 조발생률(crude rate)입니다. 사망은 희귀사건이라 사망수 D를 포아송으로 보는 것이 표준이고, D가 몇 건 안 되는 셀은 정규근사(D±1.96√D) 대신 카이제곱 분위수 기반 정확(exact) 신뢰구간을 써야 과신을 피할 수 있습니다.\n\n실무에서는 산출한 경험률을 참조율(보험개발원 참조순보험요율·경험생명표)과 A/E(실제/기대)로 비교하는 것이 상품 개발과 계리적 가정 검증(IFRS17 최선추정 가정)의 표준 절차입니다. A/E 신뢰구간이 1을 벗어나면 경험률 반영을 검토하고, 벗어나지 않으면 경험을 더 쌓거나 신뢰도(credibility) 가중으로 참조율과 결합합니다.",
    tips: "노출과 율의 기준을 섞으면(예: 중앙노출에 qx를 곱해 기대사망 계산) A/E가 체계적으로 틀어집니다. 사망 수가 적은 셀의 조발생률은 크게 출렁이므로 셀 단위 결론은 정확 CI·신뢰도 이론과 함께 — 다음 단계는 보정(graduation)입니다.",
    sections: [
      {
        title: "노출 계산과 조발생률 — 중앙 vs 초기",
        desc: "계약을 보험연도 단위로 분할해 두 가지 노출을 계산하고, 연령대(5세 구간)별 조발생률을 집계합니다. 합성 데이터라 참값(참조율×1.35)을 알고 있어 결과를 검증할 수 있습니다.",
        level: "basic",
...
print(f"전체 실제/참조(가중 A/E) = {ratio_hat:.3f} — 설정 참값 1.35와 표본오차 범위 내 일치")`,
      },
      {
        ...
        desc: "사망수가 적은 셀은 정규근사가 무너집니다 — 카이제곱 분위수의 닫힌형 공식으로 셀별 정확 구간을 계산하고, 참값 커버리지를 직접 확인합니다.",
        level: "advanced",
        code: `import numpy as np
import pandas as pd
...
ax.legend(); plt.tight_layout(); plt.show()`,
      },
      {
        ...
        desc: "기대사망 = Σ(노출×참조qx) 대비 실제사망의 비율(A/E)과 그 정확 신뢰구간으로 '경험률이 참조율과 유의하게 다른가'를 판정합니다.",
        level: "advanced",
        code: `import numpy as np
import pandas as pd
from scipy import stats
...
print("\\n※ 전체 A/E가 유의해도 구간별 CI는 대부분 1을 포함 — 셀 단위 판단에는"
  ...
  },
  {
    ...
    difficulty: 4,
    params: [
      { name: "h (평활 상수)", desc: "벌점 강도 — 작으면 원자료 밀착(과소평활, 노이즈 잔존), 크면 매끈(과잉평활, 고연령 지수 증가를 깎음). 자료의 단위·스케일에 의존하므로 여러 값을 표로 비교해 고릅니다." },
      { name: "z (차분 차수)", desc: "벌점을 거는 차분의 차수 — 관례는 2 또는 3. z=3이면 2차 곡선(포물선) 성분까지는 벌점 없이 통과시켜 완만한 곡률을 보존합니다." },
      { name: "w (가중치)", desc: "연령별 신뢰도 — 관례는 노출(경과 계약년수) 비례로 데이터가 많은 연령을 더 믿습니다. w 전체를 상수배 하면 h만 그만큼 조정된 같은 해가 나옵니다(평균 1 정규화 권장)." },
      { name: "window (이동평균 창)", desc: "중심 이동평균의 창 크기(홀수) — 클수록 매끈하지만 양끝 (window//2)개 연령이 소실되고, 볼록(지수 증가) 구간에서 위쪽 편향이 커집니다." },
      { name: "UnivariateSpline(w=, s=)", desc: "scipy 평활 스플라인 — w에 1/표준편차(사망률 로그 스케일이면 √사망수)를 주면 s≈n(자료 개수)이 관례적 출발점. s를 줄이면 매듭(knot)이 늘어 원자료에 밀착합니다." },
    ...
    intro:
      "경험데이터에서 연령별로 사망수÷노출을 계산한 조(粗)발생률은 표본이 유한한 탓에 들쭉날쭉합니다. 보정(graduation)은 '참 위험률은 연령에 대해 매끈하게 변한다'는 사전 믿음을 벌점으로 수식화해 확률 노이즈만 걷어내는 작업입니다. Whittaker-Henderson(WH)은 적합도 Σw(v−u)²(원자료 밀착)와 평활도 Σ(Δ³u)²(3차 차분 억제)를 상수 h로 저울질하는 최소화 문제로, 미분하면 선형계 (W+h·DᵀD)u=Wv 하나로 닫힌 해가 나옵니다 — 반복 계산이 필요 없습니다.\n\n한국 경험생명표(보험개발원 산출) 작성에 오랫동안 쓰여 온 고전 기법이기도 합니다. 보정된 위험률은 보험료 산출·책임준비금 평가에 그대로 들어가므로, 매끈함과 함께 중·고연령 단조 증가 같은 품질 조건까지 만족해야 실무에 쓸 수 있습니다.",
    tips: "h는 자료의 단위·스케일에 따라 최적값이 전혀 달라집니다 — 절대값을 외우지 말고 여러 h의 적합도·평활도·잔차를 비교해 고르세요. 보정 결과는 반드시 로그축 그래프(직선≈Gompertz)와 단조성 점검(np.diff(u)>0)으로 검수해야 요율에 쓸 수 있습니다.",
    sections: [
      {
        title: "합성 조발생률 + 이동평균 평활의 한계",
        desc: "매끈한 Gompertz 참값에 포아송 노이즈를 얹어 조발생률을 만들고, 가장 소박한 평활인 중심 이동평균의 구조적 한계 두 가지 — 끝단 소실·곡률(볼록) 편향 — 를 수치로 확인합니다.",
        level: "basic",
        code: `import numpy as np
import pandas as pd
...
#       '창 크기 하나'로 노이즈 제거와 모양 보존을 동시에 잡을 수 없다 → WH의 벌점 접근이 답`,
      },
      {
        ...
        desc: "평활 상수 h=100·차분 차수 z=3·노출 비례 가중치를 상수로 지정해 탐색 없이 한 번에 보정합니다. 선형계를 한 번 푸는 것이 전부 — 요철 감소·참값 RMSE 개선·단조성까지 바로 확인합니다(h를 고르는 근거는 다음 고급 섹션).",
        level: "basic",
        code: `import numpy as np
import pandas as pd
import matplotlib.pyplot as plt
...
plt.legend(); plt.tight_layout(); plt.show()`,
      },
      {
        ...
        desc: "목적함수 min Σw(v−u)² + h·Σ(Δ³u)²를 numpy 행렬로 그대로 구현합니다. 3차 차분행렬 D를 만들고 선형계 (W+h·DᵀD)u=Wv를 한 번 풀면 끝 — h=1·100·10000의 적합도 vs 평활도 트레이드오프를 표로 비교합니다.",
        level: "advanced",
        code: `import numpy as np
import pandas as pd
...
print("→ 정확도는 대등 이상 + WH는 이동평균이 못 주는 양끝 연령 보정값까지 제공")`,
      },
      {
        ...
        desc: "scipy 평활 스플라인(로그 스케일 적합)과 정확도를 비교하고, 관측 점과 보정 곡선(WH·스플라인)을 로그축에 겹쳐 그린 뒤, 요율 사용의 전제인 단조성을 확인합니다.",
        level: "advanced",
        code: `import numpy as np
import matplotlib.pyplot as plt
...
#   u_fix = IsotonicRegression(increasing=True).fit_transform(ages, u_wh)`,
  ...
  },
  {
    ...
    difficulty: 3,
    params: [
      { name: "time, event 배열", desc: "공통 입력 — 가입 후 경과월과 사건 지표(1=사망, 0=중도절단=해지·만기·관찰종료). 절단을 1로 잘못 코딩하면 사망률이 크게 부풀려집니다 — 생존분석에서 가장 흔한 치명적 실수." },
      { name: "고유 사건시점 t_i (np.unique)", desc: "S(t)는 사망이 실제 발생한 시점에서만 계단식으로 내려갑니다. 절단만 있는 시점은 곡선을 낮추지 않고 위험집합 n_i만 줄입니다 — 절단 정보를 버리지 않는 KM의 핵심 장치." },
      { name: "위험집합 n_i = Σ(time ≥ t_i)", desc: "시점 t_i 직전까지 살아 있고 절단되지 않은 계약 수. 관찰 후반에는 n_i가 얇아져 추정이 급격히 불안정해지므로 꼬리 구간 해석에 주의." },
      { name: "Greenwood 분산", desc: "Var[Ŝ(t)] = Ŝ(t)²·Σ dᵢ/(nᵢ(nᵢ−dᵢ)) — KM 추정치의 표준오차. ±1.96·SE 신뢰구간은 [0,1]을 벗어날 수 있어 클리핑하거나 log(−log) 변환 구간을 씁니다." },
      { name: "exp(−H) vs KM", desc: "Nelson-Aalen 누적위험 H(t)=Σdᵢ/nᵢ에서 S≈exp(−H). 1−x ≤ e⁻ˣ이므로 exp(−H)가 KM보다 항상 크거나 같고, 표본이 충분하면 거의 일치 — 둘의 괴리는 위험집합이 얇다는 경고 신호." },
      { name: "log-rank 가중", desc: "표준 log-rank는 모든 사건시점을 동일 가중(비례위험 대립가설에 최강력). 조기 차이에 민감하게 보려면 nᵢ 가중 Wilcoxon(Gehan) 변형 — 가중치만 바꾸면 같은 코드로 구현됩니다." },
    ],
    summary: "KM 생존곡선·Nelson-Aalen 누적위험·log-rank를 numpy로 직접 구현 — 웹 실행기에서 바로 돌아가는 생존분석",
    intro:
      "기존 '생존분석' 항목(lifelines)은 브라우저 실행기(Pyodide)에 패키지가 없어 로컬 전용이었습니다 — 이 항목은 같은 분석을 numpy만으로 직접 구현한 **웹 실행판**입니다. Kaplan-Meier는 사망이 발생한 시점마다 조건부 생존확률 (1−dᵢ/nᵢ)를 곱해 생존곡선 S(t)를 쌓아 올리고, Nelson-Aalen은 위험을 더해 누적위험 H(t)를 만들며, log-rank 검정은 두 그룹의 곡선이 통계적으로 다른지 판정합니다. 수식이 단순해 직접 구현해 보면 '중도절단을 버리지 않고 쓴다'는 생존분석의 핵심이 오히려 패키지보다 선명하게 보입니다.\n\n보험에서 중도절단(censoring)은 예외가 아니라 기본값입니다 — 해지·만기·관찰종료로 사망을 끝까지 지켜보지 못한 계약이 데이터의 절반을 넘는 것이 보통이며, 이를 그냥 제외하면 사망률이 왜곡됩니다. KM·Nelson-Aalen은 경험생명표 작성 전 원시 경험데이터의 조사망률 검토, 해지율(유지율) 곡선 산출, 그리고 log-rank는 성별·판매채널별로 위험률을 분리 산출할 근거가 있는지(제10회 경험생명표처럼 남녀 분리) 확인하는 데 바로 쓰입니다.",
    tips: "event 코딩(1=사망, 0=절단)이 뒤집히면 모든 결과가 무의미해지니 가장 먼저 검증하세요. 관찰 후반 위험집합이 수십 건 이하로 얇아진 구간의 KM 추정치는 신뢰구간이 넓어 단독 인용하지 말고, 그 구간이 필요하면 모수 모형(지수·와이블) 적합을 병행하는 것이 실무 관례입니다.",
    sections: [
      {
        title: "① Kaplan-Meier — S(t)=Π(1−dᵢ/nᵢ) 직접 구현 + Greenwood 95% CI",
        desc: "합성 경험데이터(지수분포 사망 + 해지·관찰종료 절단)로 KM을 구현하고, 이론 생존함수 exp(−t/80)이 신뢰구간에 들어오는지 검증합니다.",
        level: "basic",
        code: `import numpy as np
import matplotlib.pyplot as plt
...
plt.legend(); plt.tight_layout(); plt.show()`,
      },
      {
        ...
        desc: "같은 데이터로 누적위험 H(t)=Σdᵢ/nᵢ를 추정합니다. 상수위험(지수분포)이면 H(t)가 직선 t/80이 되어야 하므로 구현과 분포 가정을 한 번에 점검할 수 있습니다.",
        level: "advanced",
        code: `import numpy as np
import matplotlib.pyplot as plt
...
plt.tight_layout(); plt.show()`,
      },
      {
        ...
        desc: "남성 위험을 여성의 1.5배로 설계한 데이터에서 검정이 차이를 잡아내는지, 그리고 동일 위험(음성 대조)에서는 거짓 신호를 내지 않는지 함께 검증합니다.",
        level: "advanced",
        code: `import numpy as np
from scipy import stats
...
print(f"\\n[음성 대조 — 동일 위험] chi2(1) = {chi2_0:.2f},  p = {p0:.3f} (유의하지 않아야 정상)")`,
  ...
  },
  {
    ...
    difficulty: 4,
    params: [
      { name: "p · k (완전신뢰도 기준)", desc: "신뢰수준 p와 허용오차 k — '관측 빈도가 참값의 ±k 이내일 확률 p'. 관례 p=90%·k=5%면 포아송 빈도 기준 n_full=(z/k)²=1,082건. 기준을 조일수록(k↓·p↑) 필요 건수 급증." },
    ...
    intro:
      "신뢰도(credibility) 이론은 '관측된 경험을 얼마나 믿을 것인가'를 0~1 사이 숫자 Z로 정하는 계리학의 고전 이론입니다. 데이터가 충분하면 Z→1(자기 경험 위주), 부족하면 Z→0(집단 평균·매뉴얼 요율 위주)이 되고, 신뢰도 보험료는 두 추정치의 가중평균 P = Z·X̄ + (1−Z)·M입니다. 제한변동 방식은 '건수가 몇 건이면 충분한가'(완전신뢰도 1,082건)라는 건수 기반 실무 규칙이고, Bühlmann은 분산을 계약 내 잡음(EPV)과 계약 간 이질성(VHM)으로 분해해 최적 선형 추정 Z = n/(n+k)를 유도합니다.\n\n대표 용도는 단체보험·재보험 특약의 갱신 요율(경험요율)입니다 — 개별 단체의 손해 경험은 표본이 작아 그대로 쓰면 요율이 요동치고, 매뉴얼 요율만 쓰면 우량·불량 단체를 구분하지 못하므로 둘을 Z로 섞습니다. 한국 실무에서도 경험통계가 얇은 담보의 위험률을 산출할 때 보험개발원 참조순보험요율(참조위험률)을 M으로 두고 자사 경험률을 Z만큼 반영하는 접근이 같은 뿌리입니다.",
    tips: "Z는 데이터 '품질 점수'가 아니라 상대적 정보량 — 관측 잡음(EPV) 대비 계약 간 이질성(VHM)의 비율이 결정합니다. 완전신뢰도 1,082건은 포아송 '빈도' 기준이라 심도 변동까지 반영하면 (1+CV²)배로 커지고, 경험 데이터는 추세·인플레이션을 보정(on-level)한 뒤 적용해야 합니다.",
    sections: [
      {
        title: "제한변동 신뢰도 — 완전신뢰도 1,082건과 신뢰도 보험료",
        desc: "고전(limited fluctuation) 방식: '관측 빈도가 참값 ±5% 이내일 확률 90%'를 만족하는 건수(1,082건)를 완전신뢰도 기준으로 삼고, 모자라면 제곱근 법칙으로 부분신뢰도를 줍니다. 몬테카를로로 기준 자체를 검증합니다.",
        level: "basic",
        code: `import numpy as np
from scipy import stats
...
print("→ 경험이 쌓일수록(n↑) 요율이 매뉴얼에서 자사 경험 쪽으로 이동")`,
      },
      {
        ...
        desc: "단체계약 7개×6년 패널에서 분포 가정 없이 EPV(계약 내 잡음)·VHM(계약 간 이질성)을 불편추정하고 Z=n/(n+k)를 구합니다. 신뢰도 보험료가 원시 경험평균보다 참 위험도에 가까운지(MSE) 확인합니다.",
        level: "advanced",
        code: `import numpy as np
import pandas as pd
import matplotlib.pyplot as plt
...
plt.show()`,
      },
      {
        ...
        desc: "단체별 피보험자 수(노출)가 다르면 연수 대신 노출 총량 m_i가 신뢰도의 기준입니다. 노출 가중 추정량으로 계약별 Z_i와 갱신 요율표를 만들고, 포트폴리오 500개 반복으로 기대 MSE 우위를 검증합니다.",
        level: "advanced",
        code: `import numpy as np
import pandas as pd
...
print(f"\\n[{R}회 반복] 평균 MSE: 경험빈도 {se_raw/R:.3e}  vs  신뢰도빈도 {se_cred/R:.3e}"
  ...
  },
  {
    ...
    difficulty: 4,
    params: [
      { name: "개발계수 추정 방식", desc: "볼륨가중(열 전체 합의 비율 — 큰 계약이 큰 가중, 실무 표준)·단순평균·최근 n개년 평균. 포트폴리오 구성이나 지급 속도가 바뀌었다면 최근 연도 가중을 검토합니다." },
      { name: "tail factor(꼬리계수)", desc: "마지막 관측 개발연차 이후에도 남은 진전. 배상책임·장기 종목은 개발계수를 로그선형 외삽해 tail을 얹지 않으면 준비금이 과소평가됩니다(이 예제는 8년차 완전 진전 가정)." },
      { name: "사전 손해율(ELR) — BF", desc: "기대최종 = 경과보험료 × 사전 손해율. 최근 사고연도일수록 BF 결과를 지배하므로(1−1/CDF가 크므로) 요율산정·사업계획 등 출처와 갱신 주기를 문서화해야 합니다." },
      { name: "σ²_j — Mack 분산모수", desc: "개발계수의 가중잔차 분산 1/(n_j−1)·Σ c_ij(F_ij−f_j)². 마지막 연차는 잔차가 1개뿐이라 추정 불가 → 감소 패턴을 잇는 외삽 관례식 min(σ⁴/σ², min(σ², σ²))으로 채웁니다." },
      { name: "누적 vs 증분 삼각형", desc: "chain-ladder는 '누적' 삼각형 기준. 증분 자료면 np.cumsum(axis=1)으로 변환 후 적용하고, 음의 증분(환입·구상)이 잦으면 별도 검토가 필요합니다." },
      { name: "np.nan 마스킹", desc: "하삼각(아직 오지 않은 미래)은 np.nan. 개발계수의 분자·분모는 '두 칸이 모두 관측된 행'만 합산해야 하며(~np.isnan 마스크), 이걸 빼먹으면 계수가 왜곡됩니다." },
    ],
    summary:
      "런오프 삼각형의 개발 패턴으로 미지급 보험금을 추정 — Chain-Ladder 개발계수·Bornhuetter-Ferguson·Mack 표준오차",
    intro:
      "사고는 이미 발생했지만 지급이 끝나지 않은 보험금(보고 지연 IBNR 포함)을 얼마로 쌓아야 할까 — 지급준비금 추정의 고전 3종 세트입니다. 사고연도×개발연차의 런오프 삼각형에서 개발계수(전 연차 대비 누적 지급의 증가 배수)를 뽑아 미래(하삼각)를 채우는 것이 Chain-Ladder, 미보고분을 사전 손해율×보험료의 기대에 맡겨 최근 연도의 불안정을 줄이는 것이 Bornhuetter-Ferguson(BF), 그 준비금 추정치가 얼마나 흔들리는지 표준오차를 주는 것이 Mack 모형입니다.\n\nIFRS17 이후 부채 평가는 현가·위험조정 기반으로 바뀌었지만, 이 고전 기법들은 최선추정부채(BEL)의 산출·검증과 백테스트에 여전히 쓰입니다 — 국내 손해보험사의 IBNR 산출과 계리적 검증 실무에서도 chain-ladder 계열이 사실상 표준이고, 준비금 적정성 검토 보고서에는 점추정과 함께 Mack 표준오차 같은 변동성 지표를 요구하는 것이 관례입니다.",
    tips: "최근 사고연도의 CL 추정은 초기 관측 하나에 누적개발계수(이 예제는 3.8배)가 통째로 증폭되어 매우 불안정합니다 — 사전 손해율이 있으면 BF를 병행하고, 점추정만 보고하지 말고 Mack 표준오차로 불확실성을 함께 제시하세요.",
    sections: [
      {
        title: "런오프 삼각형 → 개발계수·삼각형 완성 (Chain-Ladder)",
        desc: "볼륨가중 개발계수 f_j = Σc_{i,j+1}/Σc_{i,j}로 하삼각을 채워 사고연도별 최종예상·준비금을 구합니다. NaN 마스킹(관측된 행만 합산)이 구현의 핵심.",
        level: "basic",
        code: `import numpy as np
import pandas as pd
...
print("검증) 참 최종값 대비 오차(%):", err.round(1))`,
      },
      {
        ...
        desc: "미보고분을 사전 기대(경과보험료×사전 손해율)에 맡깁니다. 최근 사고연도일수록 CL은 초기 관측 노이즈에 흔들리므로 BF와 나란히 비교하는 것이 실무 관례.",
        level: "advanced",
        code: `import numpy as np
import pandas as pd
...
print(f"\\n검증) 사전기대=CL최종일 때 BF−CL 최대오차 = "
      ...
      },
      {
        ...
        desc: "분포 가정 없이 준비금의 mse를 재귀식으로 추정해 사고연도별·총준비금 ±se를 얻습니다. 이 구현은 Mack(1993) RAA 삼각형의 공표값(총준비금 52,135·총 se 26,909)을 그대로 재현하는 공식입니다.",
        level: "advanced",
        code: `import numpy as np
import pandas as pd
import matplotlib.pyplot as plt
...
ax.legend(); plt.tight_layout(); plt.show()`,
  ...
  },
  {
    ...
    difficulty: 3,
    params: [
      { name: "alpha", desc: "sklearn GLM 공통 L2 규제 강도 — 기본값이 1.0(규제 걸림)! 요율 상대도가 목적이면 반드시 alpha=0으로 꺼야 statsmodels GLM과 같은 순수 MLE 추정치를 얻습니다." },
      { name: "power (TweedieRegressor)", desc: "분산함수 Var(Y)=φ·μ^p의 지수 — 1=포아송, 2=감마, 1<p<2가 복합 포아송–감마(0이 뭉치고 양의 연속 꼬리 = 순보험료 형태). 1.5가 관례적 출발점, 실무는 프로파일 우도·교차검증으로 선택." },
      { name: "sample_weight", desc: "노출·건수 가중 — sklearn은 offset 인자가 없어 y를 비율로 두고(건수/노출, 총액/건수, 총액/노출) 분모를 sample_weight로 주는 규약을 씁니다. 로그 링크에서 offset=log(노출)과 수학적으로 동치." },
      { name: "link", desc: "TweedieRegressor의 링크함수 — 'auto'도 log지만 명시를 권장. 로그 링크라야 exp(계수)=요율 승수(상대도)로 읽힙니다. PoissonRegressor·GammaRegressor는 항상 로그 링크." },
      { name: "fit_intercept / drop_first", desc: "절편 = 기준셀(모든 더미 0인 조합, 예: 40대·기본형)의 로그 요율 → exp(절편)=기저 요율. 원핫 시 drop_first=True로 기준 범주를 명확히 해야 상대도 표가 깔끔해집니다." },
      { name: "d·u (면책·한도)", desc: "LEV 계산의 계약 조건 — 사고당 기대 지급액 E[(X∧u)−(X∧d)]=∫ᵈᵘ S(x)dx. 면책 d를 올리면 소액 다발 사고가 걸러져 절감률이 빠르게 커집니다." },
    ],
    summary:
      "빈도(포아송)×심도(감마) 분리와 Tweedie 직접 적합으로 순보험료 산출 — 요율 상대도·오프밸런스·LEV(면책·한도)까지 요율산정 전 과정",
    intro:
      "순보험료(pure premium)는 계약 1단위(노출 1년)가 만들어낼 기대손해액입니다. 사고가 '몇 번 나는가(빈도 N)'와 '한 번에 얼마 나가는가(심도 X)'로 쪼개면 순보험료 = E[N]×E[X] — 빈도는 포아송, 심도는 감마 GLM(로그 링크)으로 각각 적합해 곱하는 것이 국제 표준이고, Tweedie 분포(1<power<2)를 쓰면 한 모형으로 직접 적합할 수도 있습니다. 로그 링크 덕분에 exp(계수)가 그대로 요율 상대도(relativity)가 되어 '20대는 기준의 1.4배' 같은 요율표가 나옵니다.\n\n이 항목은 웹 실행기에서 바로 도는 sklearn 구현입니다(이론·진단 중심의 statsmodels 구현은 'GLM' 항목 참조 — 같은 모형의 두 도구). 분리 적합 vs Tweedie 비교, 상대도 표와 오프밸런스(경영 조정 후 기저요율 보정), 면책·한도를 반영하는 LEV까지 요율산정 실무 흐름을 담았습니다. 한국 실무로는 보험개발원 참조순보험료 검증, 자동차보험 연령·차종 상대도 산출, 실손 자기부담금 개편 효과 산정이 모두 이 틀 위에 있습니다.",
    tips: "sklearn GLM의 기본 alpha=1.0(릿지)은 상대도를 1 쪽으로 수축시키므로 요율 산출은 alpha=0이 원칙입니다. 60대+처럼 계약이 적은 세그먼트는 표본 오차로 상대도가 크게 흔들리니 신뢰도(credibility) 보정이나 캡+오프밸런스로 다듬는 것이 실무 관행입니다.",
    sections: [
      {
        title: "빈도×심도 분리 적합 — PoissonRegressor·GammaRegressor(노출 처리)",
        desc: "합성 포트폴리오 600건을 만들고 빈도·심도를 따로 적합해 데이터를 만든 참값 상대도가 복원되는지 검증합니다. sklearn 노출 규약: y를 비율(건수/노출)로 두고 분모를 sample_weight로.",
        level: "basic",
        code: `import numpy as np
import pandas as pd
...
print(f"포트폴리오 검증: 예측 손해액 합 {pred_total:,.0f} vs 실제 {df['claim_amt'].sum():,.0f}만원"
      ...
      },
      {
        ...
        desc: "같은 데이터에 Tweedie 1모형으로 순보험료를 직접 적합하고 분리 적합과 산점도로 대조합니다. 45도 선 위에 모이면 두 접근이 같은 요율 구조를 봤다는 뜻.",
        level: "advanced",
        code: `import numpy as np
import pandas as pd
import matplotlib.pyplot as plt
...
plt.show()`,
      },
      {
        ...
        desc: "빈도×심도 상대도를 곱해 순보험료 상대도 표를 만들고, 경영 판단으로 상대도를 조정(캡)했을 때 총보험료를 보존하는 오프밸런스 팩터를 계산합니다.",
        level: "advanced",
        code: `import numpy as np
import pandas as pd
from sklearn.linear_model import PoissonRegressor, GammaRegressor
...
print("해석: 캡으로 깎인 60대+ 몫을 전 계약이 얇게 나눠 부담 - 세그먼트 간 내부보조가 생김")`,
      },
      {
        ...
        desc: "로그정규 심도에서 레이어 기대 지급액 E[(X∧u)−(X∧d)]를 닫힌형·quad 수치적분·몬테카를로 세 방법으로 계산해 일치를 확인하고, 면책금액별 순보험료 절감률을 냅니다.",
        level: "advanced",
        code: `import numpy as np
from scipy import stats
...
print("해석: 면책은 소액 다발 사고를 걸러 절감 효과가 크고, 한도는 꼬리 위험을 자른다")`,
  ...
  },
  {
    ...
    difficulty: 3,
    params: [
      { name: "A, B, c (Gompertz-Makeham)", desc: "사력 μx = A + B·cˣ의 3모수 — A는 연령과 무관한 배경위험(사고·재해), B·cˣ는 나이 들수록 지수적으로 커지는 노화 성분(보통 c≈1.08~1.12). qx = 1−exp(−A−B·cˣ·(c−1)/ln c)로 정확 변환하면 전 연령 단조 증가가 보장됩니다." },
      { name: "기수(radix) l0 · 종국연령 ω", desc: "l0=100,000은 생명표 작성 관례(모든 산식이 비율이라 값 자체는 임의). ω(여기서는 100세)에서 q_ω=1로 표를 '닫아야' Σdx=l0이 되고 Nx·Mx 누적합이 완결됩니다 — 닫지 않으면 종신보험 일시납이 과소평가됩니다." },
      { name: "i (예정이율) · v=1/(1+i)", desc: "v는 1년 할인계수. 예정이율은 보험료에 미리 반영하는 최소 운용수익 약속(부담금리) — i가 낮을수록 미래 보험금의 현가가 커져 보험료가 오르며, 보장기간이 길수록 민감합니다(④ 민감도 참고)." },
      { name: "Dx·Nx (생존계) / Cx·Mx (사망계)", desc: "Dx=vˣ·lx는 'x세 생존 1인의 현가', Cx=v^(x+1)·dx는 'x세 사망보험금(연말 지급)의 현가'. Nx=ΣDy·Mx=ΣCy(x 이후 누적합)이므로 연금은 N의 차, 사망보장은 M의 차로 표현됩니다." },
      { name: "α·β·γ (예정사업비 3이원)", desc: "α 신계약비(가입금액 대비, 계약 시 1회)·β 유지비(가입금액 대비, 매년)·γ 수금비(영업보험료 대비, 납입 시마다). 수지상등에 포함하면 영업보험료 G=(S·A+S·α+S·β·ä)/(ä·(1−γ))." },
      { name: "연말 지급·연납 가정", desc: "기본형 계산기수는 사망보험금 연말 지급·보험료 기시 연납을 가정한 근사입니다. 실제 월납·사망 즉시 지급 상품은 UDD 근사(월납 ä⁽¹²⁾≈ä−11/24, 즉시지급 A̅≈(1+i)^½·A)로 보정합니다." },
    ],
    summary: "생명표→계산기수(Dx·Nx·Cx·Mx)→수지상등으로 정기·종신·연금 순보험료와 3이원 영업보험료까지 — 보험료 산출의 고전 정석",
    intro:
      "생명표에서 출발해 보험료까지 가는 고전 경로를 처음부터 끝까지 구현합니다. 연령별 사망률 qx에 예정이율 할인(v=1/(1+i))을 결합해 계산기수 Dx·Nx·Cx·Mx를 한 번 만들어 두면, 정기·종신·연금 어떤 급부의 기대현가도 기수의 차와 나눗셈 몇 번으로 나옵니다. 보험료를 정하는 원칙은 수지상등(equivalence principle) 하나 — '보험료 수입의 기대현가 = 보험금·사업비 지출의 기대현가'라는 등식입니다.\n\n계산기수는 현금흐름방식(CFP) 이전 3이원방식(예정위험률·예정이율·예정사업비) 시대의 표준 도구입니다. 한국도 2013년부터 보험료 산출이 현금흐름방식으로 전환됐지만, 이 고전 산식은 지금도 표준해약환급금 산출과 산출 시스템 검증, 계리사 시험의 기본기로 살아 있습니다 — 복잡한 현금흐름 모델의 결과가 이상해 보일 때, 계산기수로 손으로 재현한 수치가 가장 믿을 만한 대조군이 됩니다.",
    tips: "계산기수 기본형은 '사망보험금 연말 지급·기시 연납' 가정의 근사라 월납·즉시지급 상품은 UDD 근사로 보정해야 실제 요율과 맞습니다. 예정이율은 회사가 최소한 달성해야 할 부담금리이므로, 저금리기에 종신처럼 보장이 긴 상품일수록 보험료가 크게 뛰는 구조(④ 민감도: 정기 +0.9% vs 종신 +13.6%)를 꼭 확인하세요.",
    sections: [
      {
        title: "① 생명표 합성 — Gompertz-Makeham qx → lx·dx (기수 100,000)",
        desc: "사력 μx = A + B·cˣ로 0~100세 qx를 합성하고(단조 증가 확인), 기수 100,000으로 lx·dx를 만듭니다. Σdx = 100,000으로 표가 제대로 닫혔는지 검증합니다.",
        level: "basic",
        code: `import numpy as np
import matplotlib.pyplot as plt
...
plt.tight_layout(); plt.show()`,
      },
      {
        ...
        desc: "생명표에 할인을 결합해 계산기수 4종을 만들고 D40·N40·M40 앵커값을 출력합니다. 재귀 항등식, 종신보험 A40 직접합산 대조(기계 정밀도 일치), 보험수리 항등식 A+d·ä=1 세 겹으로 검증합니다.",
        level: "basic",
        code: `import numpy as np
...
print(f"A40 + d*a40(기시급 종신연금) = {ident:.12f}  (이론값 1 — 잔차 {abs(ident - 1):.2e})")`,
      },
      {
        ...
        desc: "40세 가입·보험금 1억 예시로 20년 정기·종신 일시납, 기시급 생존연금 ä(40:20), 연납 순보험료를 실제 금액으로 계산합니다. 양로보험 항등식 A=1−d·ä로 검증합니다.",
        level: "basic",
        code: `import numpy as np
...
print(f"       1 − d·a(40:20)     = {ident:.10f}   (잔차 {abs(A_endow - ident):.2e})")`,
      },
      {
        ...
        desc: "예정사업비 3이원을 수지상등에 넣어 영업보험료 G를 도출하고 연간 로딩(신계약비 상각·유지비·수금비)을 분해합니다. 예정이율 0.5%p 인하 민감도에서 정기(+0.9%)와 종신(+13.6%)의 차이를 확인합니다.",
        level: "advanced",
        code: `import numpy as np
...
print("→ 같은 0.5%p 인하라도 보장이 긴 종신이 훨씬 민감 — 저금리기 종신보험료 인상의 산식적 이유")`,
  ...
  },
  {
    ...
    params: [
      ...
      { name: "u (한도) · 'u xs d' 표기", desc: "레이어 폭 — 재보험자가 클레임당 지급하는 최대액. '300 xs 200'은 200 초과분을 300까지(즉 200~500 구간) 인수한다는 뜻. 한도 소진 후 초과분은 다시 원수사 몫." },
    ...
    intro:
      "재보험은 원수보험사가 인수한 위험의 일부를 재보험자에게 넘기는 '보험회사의 보험'입니다. 비례(quota share)는 모든 클레임과 보험료를 고정 비율 q로 나누고, 비비례 XL(excess of loss)은 클레임별로 부담점 d 초과분만 한도 u까지 넘깁니다 — ceded = min(max(X−d, 0), u). 같은 재보험이라도 옮기는 위험이 다릅니다: QS는 '규모'를 줄이고(분포 모양은 그대로), XL은 '꼬리'(대형 손해)를 잘라냅니다.\n\n효과의 계량은 집합손해모형 S = X₁+…+X_N의 몬테카를로가 표준입니다 — 빈도(포아송)와 심도(로그정규 등)에서 1만 년을 시뮬레이션하고 클레임별 분출을 적용해 재보험 전후 보유 분포의 평균·VaR 99.5%·TVaR를 비교합니다. 한국 K-ICS 지급여력제도의 요구자본이 99.5% VaR 기준이므로, XL로 꼬리를 이전하면 보험리스크 요구자본이 완화되어 지급여력비율이 개선됩니다 — 재보험은 손익 안정화 수단인 동시에 자본관리 수단(최근 공동재보험까지 확장)입니다.",
    tips: "XL은 한도(d+u) 소진 후 초과분이 다시 원수사 몫이므로 보유 '최대값'을 반드시 확인하고 상위 레이어·복원(reinstatement) 조건을 검토하세요. 출재 의사결정의 기본 틀은 재보험 순비용(로딩)과 자본 절감 효과(자본비용 환산)의 비교입니다.",
    sections: [
      {
        title: "XL 레이어 분출 — 'u xs d' 기대출재손해와 LEV 검증",
        desc: "로그정규 클레임 1만 건에 3개 레이어(100 xs 100 / 300 xs 200 / 500 xs 500)를 적용해 레이어별 발동확률·기대출재·출재율 표를 만들고, 시뮬레이션 값을 닫힌형 LEV(제한기대값) 이론값으로 검증합니다.",
        level: "basic",
        code: `import numpy as np
import pandas as pd
from scipy import stats
...
#          가격(재보험료)은 '기대출재 + 변동성 로딩'으로, 상위 레이어일수록 로딩 비중이 커집니다`,
      },
      {
        ...
        desc: "같은 포트폴리오에 QS 30%와 XL '900 xs 100'을 적용해 보유 심도 프로파일(기대값·CV·99.5% 분위수)을 표로 비교합니다 — QS는 분포 모양 유지, XL은 꼬리 절단이라는 구조 차이가 핵심입니다.",
        level: "advanced",
        code: `import numpy as np
import pandas as pd
...
#   XL(대형사고·누적 위험 방어)을 목적에 따라 조합합니다`,
      },
      {
        ...
        desc: "포아송 빈도 × 로그정규 심도의 집합손해 S=ΣX를 1만 년 시뮬레이션하고 클레임별 XL을 적용해 보유/출재를 분리 — 재보험 전후 평균·VaR 99.5%·TVaR 99.5% 비교표와 보유 분포 히스토그램(전후 겹침), 자본 절감 대비 순비용까지 계산합니다.",
        level: "advanced",
        code: `import numpy as np
import pandas as pd
import matplotlib.pyplot as plt
