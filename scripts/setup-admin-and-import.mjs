...
const ITEMS = [
  {
    ...
    desc: "손해보험 비용담보 최초의 ‘변호사선임 현물급부’ 사례. 2026년 변호사선임비용 규제 강화(자기부담 50%·심급별 한도 분리)에 현금이 아닌 제휴 법무법인의 현물 제공으로 대응한 구조의 독창성·진보성과, 변호사법 제34조(소개·알선 금지) 리스크를 정밀 진단.",
  },
  {
  ...
  },
  {
    ...
    desc: "받은 보험금을 뺀 납입보험료를 환급연령(가입연령별 60~85세) 생존 시 돌려주는 국내 최초 청구연동 개별 환급형 건강보험. 신규성은 위험률이 아니라 청구연동 환급(claims-linked ROP)과 이를 구현하는 retrospective 적립 시스템에 있음. 손실회피 고객 정조준·소액청구 억제(손해율 순기능)가 강점이나, 저축성 결합 수익성·불완전판매 민원 리스크 진단. 생명보험협회 9개월 배타권.",
  },
  {
  ...
  },
  {
    ...
    desc: "급여 심폐소생술(CPR)·제세동·전기적심조율전환 시행 시 가입금액 100%를 1회당 정액 지급. 진단·수술 이전의 응급처치(救命) 영역을 모든 원인·면책·횟수 제한 없이 정액 보장하는 업계 최초 특약. 수가코드 연동 정액(수술비 정액담보)의 변형이라 진보성·카피 취약, 준임종 급부(생존율 9.2%)와 1일 복수코드 지급배수가 손해율 좌우.",
  },
  {
    file: "분석보고서_교보생명_특정자궁질환보장특약.pdf",
    title: "교보생명 특정자궁질환보장특약(급여 초음파 검사비) 분석",
    desc: "다빈도 여성 자궁질환(근종 D25·기타 양성신생물 D26·자궁내막증 N80) 진단비(최초 1회 50만)와 급여 초음파 검사비(연 1회 10만)를 정액 보장. 신규성은 진단비가 아니라 ‘급여 초음파 검사비’로, 보장 축을 확진 이후에서 예방·조기발견으로 전진. HIRA 급여 데이터 기반 산출이 강점이나 검사비 청구 인플레·실손 중복·검사비 정액 카피 용이가 리스크. 6개월 배타권.",
  },
  {
    file: "분석보고서_라이나생명_미세잔존암WGS검사지원형.pdf",
    title: "라이나생명 암생존지원특약(미세잔존암 WGS검사 지원형) 분석",
    desc: "암 최초진단 이후 최대 10년간 매년 생존 시 미세잔존암(MRD) WGS검사를 현물(또는 현금 175만) 지급하는 업계 최초 생존조건부 검사지원 특약. 진단·치료의 다음 단계인 치료 후 추적관찰 공백을 정조준, 전장유전체 기반 ctDNA 분석의 진보성. 생존율 높을수록 누적 지급이 커지는 역방향 비용구조·제3자(Inocras) 벤더 의존 카피·현금전환 모럴해저드 진단.",
  },
  {
    file: "분석보고서_신한라이프_신한톤틴연금보험.pdf",
    title: "신한라이프 신한톤틴연금보험(생존자 재분배) 분석",
    desc: "조기 사망·해지자의 적립 차익(톤틴효과+저해지효과)을 생존자 연금재원에 재분배해 장수할수록·개시를 늦출수록 연금이 커지는(70세 개시 시 일반형 대비 138%) 국내 최초 한국형 톤틴연금. 사망·해지 크레딧 풀링이 본질, 원금손실 제거(사망≥납입·10년 후 해약 100%)가 일본 선례 대비 진보. 장수 역선택·유지율 의존이 핵심 리스크. 생명보험협회 12개월 배타권(생보 역대 두 번째).",
  },
  {
